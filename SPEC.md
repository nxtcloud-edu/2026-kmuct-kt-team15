...
| `semesters` | 정수 0~8 (마친 학기). 화면 선택지는 1~8이다(9/20, 0은 고를 값이 아니라서 뺐다) | 온보딩 |
...
- **되묻기 카드 (홈):** "오늘 할 일" 카드와 분야 칩 사이에 둔다. `GET /api/ask` 결과이고, 명세 3절의 카드와 선택 칩 모양을 재사용한다.
  - 라벨 "정보를 더 입력하면 {unlock}건 더 판단할 수 있어요", 질문, 선택지 칩, "모르겠어요"를 둔다.
...
- **목록 행의 "계획 중":** 명세 3절의 부제는 통째로 muted라 "계획 중"이 D-day, 부서와 섞여 안 보인다. 이 조각만 강조색에 굵기 700을 준다. 옆의 D-day는 마감 3일 안에서만 `--urgent`가 되므로 색이 겹치지 않는다.
- **묶음 박스 (홈):** 시안은 목록 카드 하나 안에 지원 가능 행을 두고 나머지 묶음을 접기 버튼으로 숨긴다. 여기서는 8.1의 네 묶음이 각자 카드가 된다(명세 3절의 카드 모양, 카드 사이 12px). 접기 버튼은 없애고 늘 펴 둔다. 다 펴면 목록이 길어지는 것은 박스 안 세로 스크롤로 막는다.
  - 순서는 시안 그대로다: 지원 가능 → "정보를 입력하면 판단할 수 있는 공지 {n}건" → "아직 확인 중인 공지 {n}개" → "조건이 안 맞는 공지 {n}개". 시안의 접기 버튼 문구 "정보를 더 입력하면 판단할 수 있어요 {n}건"은 문장이라 나머지 둘(명사구)과 결이 달랐다. 머리글 셋을 "… 공지 {n}건/개" 한 꼴로 맞춘다.
...
- 시안의 "캘린더 앱에 모두 추가"(명세 5.8)는 빼고 그리지 않는다. 실제로 캘린더에 넣지 않아서 누르면 "(시안)"이라고 적힌 토스트만 떴다. 데모에서 이것만 가짜라서 다른 버튼까지 시늉으로 보이게 한다. 토스트 문구 "캘린더 앱에 추가했어요 (시안)"도 함께 사라진다.
...
- **기억은 한 턴:** 게이트웨이는 호출 사이를 기억하지 않는다. 서버가 학생마다 직전 질문·답(300자)·답의 공지 key를 프로세스 메모리에 두고 다음 질문의 세 번째 메시지 앞머리에 "직전 질문/직전 답"으로 붙인다(재시작하면 잊는다). 그보다 앞선 대화는 넘기지 않는다. refs와 지원 가능 여부는 여전히 이번 질문의 도구 결과로만 정한다(미결 질문 7번, 9/20 결정).
...
- **목록 안전망 (9/20):** 답 없이 끝났는데 이번 질문에서 `notice_keys`·`overrides` 없는 `check_eligibility`가 지원 가능 공지를 1건 이상 돌려줬으면, 그 결과로 서버가 답을 만든다: "지금 지원할 수 있는 공지는 N건입니다. {제목 6개}{외 M건}이 있습니다.", refs는 그 key 전부. `chat_miss`에는 적지 않는다. 화면의 "지원할 수 있는 공지는 N개"를 가리키는 "그 N개가 뭐예요?"가 배포 서버에서 "찾을 수 없어요"로 끝난 뒤 넣었다. 세 번째 system 메시지 앞머리에는 그 화면 숫자도 적어 준다("학생 화면에는 … N개라고 적혀 있다").

**도구.** 결과는 짧게 잘라 system 메시지에 붙인다.

| 도구 | 하는 일 | 화면 단계 이름 |
|---|---|---|
| `search_notices(query, category?)` | 점검된 카드 전체에서 제목 3배 가중 + 부제·요약·원문의 글자 bigram BM25로 상위 10건. 지난 공지도 `open` 표시와 함께 돌려준다. 결과는 `{"items", "total"(맞는 카드 전체 수), "shown"}`이고 단계 설명은 상한에 걸리면 "N건 중 10건"이다 | 공지 검색 |
| `get_profile()` | 학생 정보 | 내 정보 불러오기 |
| `check_eligibility(notice_keys?, overrides?, category?)` | `judge` 호출. 가정 질문은 overrides로. `notice_keys`가 없으면 열린 공지 전체(`category`를 주면 그 분야만)를 판정해 지원 가능한 것만 마감순으로 20건까지 준다. `notice_keys`를 주면 결과마다 `conditions`와 그 카드의 준비할 것 `tasks: [{"title", "due"}]`가 붙는다(9/20, "뭘 준비해야 해?"에 답하려고) | 조건 대조 |
| `get_plan()` | 계획한 카드와 남은 할 일 | 내 계획 불러오기 |

- **조회 범위:** `check_ok = 1`, `hidden = 0`, `linked_to IS NULL`, 게시일 ≤ 오늘인 카드. `demo_new`는 공개한 학생만 본다. 도구 넷 모두 이 범위만 본다(`get_plan`도 계획에 넣은 뒤 숨기거나 연결된 카드는 빼고, 그 카드의 할 일도 세지 않는다). `open`은 목록의 신청 기간 조건을 만족하는지다. `search_notices`는 모르는 category를 무시하고, query가 비면 열린 공지를 마감순으로 10건 준다. `check_eligibility`의 `overrides`는 `PATCH /api/me`와 같은 검증을 거친다. `notice_keys`는 1~10개이거나 null이고, 범위 밖 key는 결과의 `not_found`에 적는다. 모두 범위 밖이면 잘못된 인자다. null이면 결과는 `{"summary": "모집 중인 공지 N건 중 M건에 지금 지원할 수 있다", "checked", "eligible", "results": [{"key", "title", "apply_end", "days_left", "category"}]}`이고 단계 설명은 "N건 중 M건 지원 가능"이다. "지금 지원할 수 있는 것", "받을 수 있는 장학금", "이번 달 마감"처럼 공지를 정하지 않은 질문은 검색 결과만으로는 답할 수 없어서 더했다(시드 학생은 마감이 가까운 5건이 모두 지원 불가이고, 지원 가능한 첫 카드가 9번째다).
- **도구 결과 모양.** 카드 한 건은 `{"key", "title", "category", "apply_end", "days_left", "open"}`이다. `search_notices`는 `{"items": [카드], "total", "shown"}`, `get_profile`은 `{"profile"}`, `get_plan`은 `{"items": [카드 + "tasks": [{"title", "due", "done"}]], "remaining_tasks"}`다. `notice_keys`를 준 `check_eligibility`는 `notice_keys`가 null일 때와 같은 껍데기(`summary`, `checked`, `eligible`, `results`)에 카드마다 `eligible`, `gap`, `conditions`(`label`, `need`, `status`, `have`)를 더하고 `not_found`를 붙인다. 결과 JSON은 5,000자에서 자른다(9/20: 1,200자는 키 없는 `check_eligibility` 12건을 9건에서 잘랐고, 키 10건 + `conditions` + `tasks`가 약 3,900자다).
...
2. 홈: "지원할 수 있는 기회가 N개 있어요"와 조건 칩(색, 툴팁)이 보인다. 첫 진입은 관심 분야로 필터가 걸리므로 N은 걸린 분야 안의 수이고, "전체 X개 중"의 X가 `eligible_count`와 같다. 아래에 "정보를 입력하면 판단할 수 있는 공지 M건", "아직 확인 중인 공지 P개", "조건이 안 맞는 공지 K개"가 있다.
...
| 9/20 사용자 결정: "캘린더 앱에 모두 추가"를 뺀다. 목업으로 두면 데모에서 이것만 가짜다 | 17번을 바꿈 | 8.3 |
...
7. ~~대화를 다중 턴으로 할지~~ 9/20 결정: 직전 한 턴만 문맥으로 넘긴다(8.4 "기억은 한 턴"). 리허설에서 "그럼 나머지 2건은 뭐야?"가 문맥 없이 "찾을 수 없어요"로 끝났다.
