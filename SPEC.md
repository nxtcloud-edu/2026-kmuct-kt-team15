...
- **`ask`:** `field`가 있을 때만 문구, 없으면 null이다. 되묻기 키는 아래 질문 틀이다. 온보딩 키는 UI 시안 문구다:
  ...
  - `langs`: "{any_of 첫 시험}이나 {둘째 시험} 성적 갖고 계신가요?" (시험이 하나면 "{시험} 성적 갖고 계신가요?")
...
**알림.** 세 종류이고 하나도 없으면 빈 배열이다. 아래 순서로 쌓는다. 문구는 `title` / `body`다.
...
- **확인 중인 공지 묶음:** 8.1의 4번 묶음을 홈 목록 카드 안에 따로 접어 둔다. 명세 3절의 접기 버튼 모양을 그대로 쓰고, 문구는 "아직 확인 중인 공지 {n}개", 자리는 "정보를 더 입력하면" 버튼과 "조건이 안 맞는 공지" 버튼 사이다. 행의 부제와 상세 판정 문구는 첫 비통과 행의 `gap`이고, 상세 아래 버튼은 그 `gap`을 쓴 비활성 버튼(시안의 지원 불가 버튼 모양)이다.
...
- **원문 시트:** 제목 아래 `path` 한 줄을 둔다. 출처별 구역마다 "출처 열기" 링크를 달고, `highlights` 구간을 `<mark>`로 강조하고, 열 때 첫 강조로 스크롤한다. 시안의 게시판 링크와 닫기 버튼은 그대로 둔다.
- **새 공지 확인:**
...
- 운영자 API는 7절 계약 밖이고 `/admin` 아래라 같은 비밀번호로 막힌다.
